...
const bookCtrl = {
  createBookingRequest: async (req, res) => {
    try {
      ...
      const { startDate,endDate, bookingTime } = req.body;
      console.log(req.body);

      if (!roomId || !startDate ||!endDate||!bookingTime) {
      ...
      await sendEmail(
        ...
        `
          <h3>your booking request has been sent successfully on ${startDate,endDate} at ${bookingTime}.</h3>
          <p>Owner Details:</p>
          <p>
      ...
      await sendEmail(
        ...
        "New Booking Request",
        `
          <h3>You have a new booking request for your room on ${startDate,endDate} at ${bookingTime}.</h3>
          <p>User Details:</p>
          <p>
  ...
  try {
    const { bookingId } = req.params;
    const booking = await Book.findById(bookingId).populate("user").populate("room");
    ...
    const emailContent = `
      ...
      <p><a href="${formLink}">Fill Out Affidavit Form</a></p>
    `;
    await sendEmail(booking.user.email, "Booking Accepted - Fill Out the Affidavit Form", emailContent);

    res.send("Booking accepted, room unavailable, and affidavit form email sent");
  } catch (error) {
    console.error(error.message);
    res.status(500).send("Server error");
  }
    ...
    try {
      ...
      try {
        await sendEmail(
          ...
          `You have a new affidavit submission for your room booking on ${booking.startDate,booking.endDate} at ${booking.bookingTime}.`
        );
