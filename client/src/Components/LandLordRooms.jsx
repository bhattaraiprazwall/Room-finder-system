...
const LandLordRooms = () => {
  ...
  const fetchRooms = async () => {
    ...
    try {
      ...
      console.log("API Response:", res.data); // Log full response to inspect structure

      const roomData = res?.data || []; // Prevent undefined error
      setRoom(roomData);
    } catch (error) {
      if (error.response) {
        ...
        toast.error(
          `Error: ${error.response.status} - ${
            error.response.data?.msg || "Failed to fetch rooms"
          }`
        );
      } else {
